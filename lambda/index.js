...
const WhereAmIIntentHandler = {
  ...
  handle(handlerInput) {
    var speakOutput = 'I have no idea where you are.';

    const context  = handlerInput.requestEnvelope.context;

    // Does the device support geo-location?
    ...
    if (isGeoSupported ) {
        if (context.Geolocation && context.Geolocation.coordinate) {
            ...
            return handlerInput.responseBuilder
                .speak('I need to know where you are. Please enable location services in the Alexa app.')
                .withAskForPermissionsConsentCard(['alexa::devices:all:geolocation:read'])
                .getResponse();
        }
    } else {
        speakOutput = 'Your device does not support GeoLocation. Please use a mobile device to ask about your location.';
    }
